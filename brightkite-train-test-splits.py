...
brightkite_adj = pickle.load(open('./brightkite/brightkite-adj.pkl', 'rb'))
...
for frac_hidden in FRAC_EDGES_HIDDEN:
    ...
    train_test_split = mask_test_edges(brightkite_adj,
        test_frac=test_frac, val_frac=val_frac, 
        verbose=True)

    # Save split
    file_name = TRAIN_TEST_SPLIT_DIR + 'brightkite-{}-hidden.pkl'.format(frac_hidden)
