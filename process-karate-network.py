...
karate_edges_dir = './karate/karate.txt'

# Parse edgelist into directed graph
# edges_f = open(karate_edges_dir)
with open(karate_edges_dir, 'rb')as edges_f:
    karate_g = nx.read_edgelist(edges_f, nodetype=int, create_using=nx.DiGraph(), encoding='latin1')
print('Num. weakly connected components: ', nx.number_weakly_connected_components(karate_g))

# print('Saving adjacency matrix')

# Get adjacency matrix
adj = nx.adjacency_matrix(karate_g)

# Save adjacency matrix
with open('./karate/karate-adj.pkl', 'wb') as f:
    pickle.dump(adj, f)
...
def save_network_statistics(g):
    ...
    with open('./network-statistics/karate-statistics.txt', 'wb') as f:
        for stat_name, stat_value in stats.items():
            f.write(stat_name + ': ' + str(stat_value) + '\n')


print('Generating network visualization')
# save_visualization(g=karate_g, file_name='./visualizations/karate-combined-visualization.pdf', title='karate Combined Network')

print('Calculating and saving network statistics')
save_network_statistics(karate_g)
