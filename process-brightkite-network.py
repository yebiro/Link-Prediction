...
brightkite_edges_dir = './brightkite/brightkite.txt'

# Parse edgelist into directed graph
# edges_f = open(brightkite_edges_dir)
with open(brightkite_edges_dir, 'rb')as edges_f:
    brightkite_g = nx.read_edgelist(edges_f, nodetype=int, create_using=nx.DiGraph(), encoding='latin1')
print('Num. weakly connected components: ', nx.number_weakly_connected_components(brightkite_g))

# print('Saving adjacency matrix')

# Get adjacency matrix
adj = nx.adjacency_matrix(brightkite_g)

# Save adjacency matrix
with open('./brightkite/brightkite-adj.pkl', 'wb') as f:
    pickle.dump(adj, f)
...
def save_network_statistics(g):
    ...
    with open('./network-statistics/brightkite-statistics.txt', 'wb') as f:
        for stat_name, stat_value in stats.items():
            f.write(stat_name + ': ' + str(stat_value) + '\n')


print('Generating network visualization')
# save_visualization(g=brightkite_g, file_name='./visualizations/brightkite-combined-visualization.pdf', title='brightkite Combined Network')

print('Calculating and saving network statistics')
# save_network_statistics(brightkite_g)
