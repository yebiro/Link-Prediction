...
advogato_edges_dir = './advogato/advogato.txt'

# Parse edgelist into directed graph
# edges_f = open(advogato_edges_dir)
with open(advogato_edges_dir, 'rb')as edges_f:
    advogato_g = nx.read_edgelist(edges_f, nodetype=int, create_using=nx.DiGraph(), encoding='latin1', data=(('weight', float),))
print('Num. weakly connected components: ', nx.number_weakly_connected_components(advogato_g))

# print('Saving adjacency matrix')

# Get adjacency matrix
adj = nx.adjacency_matrix(advogato_g)

# Save adjacency matrix
with open('./advogato/advogato-adj.pkl', 'wb') as f:
    pickle.dump(adj, f)
...
def save_network_statistics(g):
    ...
    with open('./network-statistics/advogato-statistics.txt', 'wb') as f:
        for stat_name, stat_value in stats.items():
            f.write(stat_name + ': ' + str(stat_value) + '\n')


print('Generating network visualization')
# save_visualization(g=advogato_g, file_name='./visualizations/advogato-combined-visualization.pdf', title='advogato Combined Network')

print('Calculating and saving network statistics')
# save_network_statistics(advogato_g)
