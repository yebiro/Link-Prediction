...
gplus_edges_dir = './gplus/gplus.txt'
...
adj = nx.adjacency_matrix(gplus_g)

# Save adjacency matrix
with open('./gplus/gplus-adj.pkl', 'wb') as f:
    pickle.dump(adj, f)
...
def save_network_statistics(g):
    ...
    with open('./network-statistics/gplus-statistics.txt', 'wb') as f:
        for stat_name, stat_value in stats.items():
            f.write(stat_name + ': ' + str(stat_value) + '\n')


print('Generating network visualization')
# save_visualization(g=gplus_g, file_name='./visualizations/gplus-combined-visualization.pdf', title='gplus Combined Network')

print('Calculating and saving network statistics')
# save_network_statistics(gplus_g)
