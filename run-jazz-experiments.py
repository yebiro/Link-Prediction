...
# Read in jazz network
TRAIN_TEST_SPLITS_FOLDER = './train-test-splits/'
network_dir = './jazz/jazz-adj.pkl'
jazz_adj = None
with open(network_dir, 'rb') as f:
    jazz_adj = pickle.load(f)



### ---------- RUN LINK PREDICTION TESTS ---------- ###
for i in range(NUM_REPEATS):
    jazz_results = {} # nested dictionary: experiment --> results

    # Check existing experiment results, increment file number by 1
    past_results = os.listdir('./results/')
    experiment_num = 0
    experiment_file_name = 'jazz-experiment-{}-results.json'.format(experiment_num)
    while (experiment_file_name in past_results):
        experiment_num += 1
        experiment_file_name = 'jazz-experiment-{}-results.json'.format(experiment_num)

    jazz_results_dir = './results/' + experiment_file_name
    ...
    for frac_hidden in FRAC_EDGES_HIDDEN:
        ...
        experiment_name = 'jazz-{}-hidden'.format(frac_hidden)
        ...
        jazz_results[experiment_name] = lp.calculate_all_scores(jazz_adj, features_matrix=None, 
                                                     directed=False, \
        ...
        with open(jazz_results_dir, 'w') as fp:
            json.dump(jazz_results, fp, indent=4)
