...
hamster_adj = pickle.load(open('./hamster/hamster-adj.pkl', 'rb'))
...
for frac_hidden in FRAC_EDGES_HIDDEN:
    ...
    train_test_split = mask_test_edges(hamster_adj,
        test_frac=test_frac, val_frac=val_frac, 
        verbose=True)

    # Save split
    file_name = TRAIN_TEST_SPLIT_DIR + 'hamster-{}-hidden.pkl'.format(frac_hidden)
