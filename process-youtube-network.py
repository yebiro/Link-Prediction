...
youtube_edges_dir = './youtube/youtube.txt'

# Parse edgelist into directed graph
# edges_f = open(youtube_edges_dir)
with open(youtube_edges_dir, 'rb')as edges_f:
    youtube_g = nx.read_edgelist(edges_f, nodetype=int, create_using=nx.DiGraph(), encoding='latin1')
print('Num. weakly connected components: ', nx.number_weakly_connected_components(youtube_g))

# print('Saving adjacency matrix')

# Get adjacency matrix
adj = nx.adjacency_matrix(youtube_g)

# Save adjacency matrix
with open('./youtube/youtube-adj.pkl', 'wb') as f:
    pickle.dump(adj, f)
...
def save_network_statistics(g):
    ...
    with open('./network-statistics/youtube-statistics.txt', 'wb') as f:
        for stat_name, stat_value in stats.items():
            f.write(stat_name + ': ' + str(stat_value) + '\n')


print('Generating network visualization')
# save_visualization(g=youtube_g, file_name='./visualizations/youtube-combined-visualization.pdf', title='youtube Combined Network')

print('Calculating and saving network statistics')
# save_network_statistics(youtube_g)
