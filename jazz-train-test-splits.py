...
jazz_adj = pickle.load(open('./jazz/jazz-adj.pkl', 'rb'))
...
for frac_hidden in FRAC_EDGES_HIDDEN:
    ...
    train_test_split = mask_test_edges(jazz_adj,
        test_frac=test_frac, val_frac=val_frac, 
        verbose=True)

    # Save split
    file_name = TRAIN_TEST_SPLIT_DIR + 'jazz-{}-hidden.pkl'.format(frac_hidden)
