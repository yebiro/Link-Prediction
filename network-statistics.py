...
######### READ EDGE LIST #########
##################################
networks = tuple(['advogato', 'gplus', 'hamster', 'jazz', 'karate'])

for network in networks:
    print('Reading edgelist')
    print(network)
    # Read combined edge-list
   # network_edges_dir = './{0}/{1}.txt'.format(network).format(network)
    network_edges_dir= './'+network+'/'+network+'.txt'
    print(network_edges_dir)
    # Parse edgelist into directed graph
    # edges_f = open(network_edges_dir)
    with open(network_edges_dir, 'rb')as edges_f:
        network_g = nx.read_edgelist(edges_f, nodetype=int, create_using=nx.DiGraph(), encoding='latin1', data=(('weight', float),))
    ...
    def save_network_statistics(g):
        ...
        stats['avg_degree'] = sum(dict(g.degree()).values()) / float(stats['num_nodes'])
        ...
        with open('./network-statistics/'+network+'-statistics.txt', 'w') as f:
            for stat_name, stat_value in stats.items():
                f.write(stat_name + ': ' + str(stat_value) + '\n')


    print('Generating network visualization')
    # save_visualization(g=network_g, file_name='./visualizations/network-combined-visualization.pdf', title='network Combined Network')

    print('Calculating and saving network statistics')
    save_network_statistics(network_g)
