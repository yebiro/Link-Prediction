...
hamster_edges_dir = './hamster/hamster.txt'

# Parse edgelist into directed graph
# edges_f = open(hamster_edges_dir)
with open(hamster_edges_dir, 'rb')as edges_f:
    hamster_g = nx.read_edgelist(edges_f, nodetype=int, create_using=nx.DiGraph(), encoding='latin1')
print('Num. weakly connected components: ', nx.number_weakly_connected_components(hamster_g))

# print('Saving adjacency matrix')

# Get adjacency matrix
adj = nx.adjacency_matrix(hamster_g)

# Save adjacency matrix
with open('./hamster/hamster-adj.pkl', 'wb') as f:
    pickle.dump(adj, f)
...
def save_network_statistics(g):
    ...
    with open('./network-statistics/hamster-statistics.txt', 'wb') as f:
        for stat_name, stat_value in stats.items():
            f.write(stat_name + ': ' + str(stat_value) + '\n')


print('Generating network visualization')
# save_visualization(g=hamster_g, file_name='./visualizations/hamster-combined-visualization.pdf', title='hamster Combined Network')

print('Calculating and saving network statistics')
# save_network_statistics(hamster_g)
