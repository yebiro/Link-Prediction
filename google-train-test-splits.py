...
google_adj = pickle.load(open('./google/google-adj.pkl', 'rb'))
...
for frac_hidden in FRAC_EDGES_HIDDEN:
    ...
    train_test_split = mask_test_edges_directed(google_adj,
        test_frac=test_frac, val_frac=val_frac, 
        verbose=True, prevent_disconnect=False, false_edge_sampling='random')

    # Save split
    file_name = TRAIN_TEST_SPLIT_DIR + 'google-{}-hidden.pkl'.format(frac_hidden)
