...
jazz_edges_dir = './jazz/jazz.txt'

# Parse edgelist into directed graph
# edges_f = open(jazz_edges_dir)
with open(jazz_edges_dir, 'rb')as edges_f:
    jazz_g = nx.read_edgelist(edges_f, nodetype=int, create_using=nx.DiGraph(), encoding='latin1')
print('Num. weakly connected components: ', nx.number_weakly_connected_components(jazz_g))

# print('Saving adjacency matrix')

# Get adjacency matrix
adj = nx.adjacency_matrix(jazz_g)

# Save adjacency matrix
with open('./jazz/jazz-adj.pkl', 'wb') as f:
    pickle.dump(adj, f)
...
def save_network_statistics(g):
    ...
    with open('./network-statistics/jazz-statistics.txt', 'wb') as f:
        for stat_name, stat_value in stats.items():
            f.write(stat_name + ': ' + str(stat_value) + '\n')


print('Generating network visualization')
# save_visualization(g=jazz_g, file_name='./visualizations/jazz-combined-visualization.pdf', title='jazz Combined Network')

print('Calculating and saving network statistics')
# save_network_statistics(jazz_g)
