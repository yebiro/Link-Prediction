...
karate_adj = pickle.load(open('./karate/karate-adj.pkl', 'rb'))
...
for frac_hidden in FRAC_EDGES_HIDDEN:
    ...
    train_test_split = mask_test_edges(karate_adj,
        test_frac=test_frac, val_frac=val_frac, 
        verbose=True)

    # Save split
    file_name = TRAIN_TEST_SPLIT_DIR + 'karate-{}-hidden.pkl'.format(frac_hidden)
